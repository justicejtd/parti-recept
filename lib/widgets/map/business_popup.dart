      ...
      child: InkWell(
        onTap: () => setState(() {
          Clipboard.setData(new ClipboardData(
                  text:
                      '${this._marker.streetName} ${this._marker.streetNumber}${this._marker.streetSuffix.length > 0 ? '(${this._marker.streetSuffix})' : ""}${this._marker.postcode.length > 0 ? ', ${this._marker.postcode}' : ""}'))
              .then((_) {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              'Address: ${this._marker.streetName} ${this._marker.streetNumber}${this._marker.streetSuffix.length > 0 ? '(${this._marker.streetSuffix})' : ""}${this._marker.postcode.length > 0 ? ', ${this._marker.postcode}' : ""}',
              style: const TextStyle(fontSize: 12.0),
            ),
            Text(
              this._marker.postcode.length > 0
                  ? 'Postcode: ${this._marker.postcode}'
                  : "",
              style: const TextStyle(fontSize: 12.0),
            ),
            new InkWell(
                child: this._marker.website.length > 0
                    ? formatURL((this._marker.website))
                    : new Text(
                        "Website: N/A",
                        style: const TextStyle(fontSize: 12.0),
                      ),
                onTap: () => launch('${this._marker.website}')),
          ],
        ),
      ),
    );
